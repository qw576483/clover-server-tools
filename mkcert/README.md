...
| `mkcert-v1.4.4-windows-amd64.exe` | mkcert 可执行文件，**已随仓库**（clone 后直接可用，无需下载）。仅当要换版本时，才从 [mkcert releases](https://github.com/FiloSottile/mkcert/releases) 下载放入本目录 —— `.gitignore` 已按 `mkcert/mkcert-*.exe` 放行 |
...
# 0. mkcert 可执行文件已随仓库，直接用即可（无需下载）。
#    仅当要换版本时：从 https://github.com/FiloSottile/mkcert/releases
#    下载后放入本目录，并同步改下面命令里的文件名。
