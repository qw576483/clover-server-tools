...
// Request / Reply / Notify 结构体字段，供 msg-web 的 Web GUI 使用。
//
// 解析规则与 msg-client/internal/proto/parse.go 保持一致。
package proto
...
// 硬编码引擎内核消息定义。
// 引擎消息稳定且数量少，内置后 msg-web 不再需要读取引擎源码目录，
...
func (idx *Index) scanFile(path, source string) error {
	...
	for _, m := range reConst.FindAllStringSubmatch(src, -1) {
		...
		if !strings.HasPrefix(name, "EMsg") && !strings.HasPrefix(name, "EReply") &&
			...
			!strings.HasPrefix(name, "ReplyMsg") && !strings.HasPrefix(name, "PushMsg") &&
			!strings.HasPrefix(name, "Push") {
			continue
		}
		if n > math.MaxUint32 {
			continue
...
// 嵌套结构体字段类型在自动生成 JSON 时跳过（只处理基础类型字段）。
...
func (idx *Index) associate() {
	for _, msg := range idx.msgs {
		...
		case strings.HasPrefix(base, "PushMsg"):
			base = base[len("PushMsg"):]
		...
		// 消息名前缀决定方向优先：ReplyMsg/EReply → s2c，PushMsg/EPush/Push → s2c
		switch {
		case strings.HasPrefix(msg.Name, "ReplyMsg") || strings.HasPrefix(msg.Name, "EReply"):
			msg.Dir = "s2c"
		case strings.HasPrefix(msg.Name, "PushMsg") || strings.HasPrefix(msg.Name, "EPush") || strings.HasPrefix(msg.Name, "Push"):
			msg.Dir = "s2c"
...
func (idx *Index) fieldsOf(structName string) []Field {
	if s, ok := idx.structs[structName]; ok {
		// 过滤掉嵌套结构体字段（如 time.Time），只返回基础类型
		var filtered []Field
